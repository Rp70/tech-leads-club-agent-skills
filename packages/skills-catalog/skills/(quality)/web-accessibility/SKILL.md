...
**A same-value `aria-label` doesn't fix a generic-text link for a screen reader — and doesn't touch Lighthouse's `link-text` SEO audit at all.** For real accessibility (WCAG 2.4.4, the computed *accessible name*), `aria-label` fully *overrides* visible text rather than supplementing it — so `<a aria-label="Learn more">Learn more</a>` still has the accessible name "Learn more", and the fix is to make the override's *value* more descriptive than the visible label (`aria-label={`Learn more: ${event.title}`}`), not to add an override that echoes it. This is easy to miss on a component that already sets `aria-label={title}` defensively (e.g. a shared `ButtonCTA`), since the prop *looks* like an accessibility fix already in place.

But that `aria-label` fix, correct as it is for WCAG, does **not** satisfy Lighthouse's separate `link-text`/"Links do not have descriptive text" *SEO* audit, which checks `node.innerText` against a generic-phrase blocklist and never reads `aria-label` (verified from `lighthouse/core/audits/seo/link-text.js` — it filters on `link.text`, sourced purely from `innerText`). If that audit is also failing, the rendered text itself has to change: add a `.sr-only` span *inside* the link with the differentiating context, e.g. `<a aria-label="Learn more: ...">Learn more<span class="sr-only"> {event.title}</span></a>`. `.sr-only` (absolute-positioned + clipped, not `display:none`/`visibility:hidden`) is still part of `innerText`, so the audit's exact-match check no longer matches "learn more" while sighted users still see only "Learn more" — see [SEO § Accessibility & SEO overlap](../seo/SKILL.md#8-accessibility--seo-overlap) for the audit-mechanics detail. Don't assume the `aria-label` fix alone cleared this audit; re-run Lighthouse and check `link-text`'s `details` specifically.
